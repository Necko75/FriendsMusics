...
var PlayList = { 
	...
	**/
	...
	/* params.id_playlist, params.pattern */
	findPatternIn : function(session, params, callback) {
		var query = 'SELECT * FROM musics WHERE musics.playlist_id = "'+params.id_playlist+'" AND (LOWER(author) LIKE "%'+params.pattern+'%" OR LOWER(name) LIKE "%'+params.pattern+'%")';
		DB.query(query, function(err, rows, fields)
		{
			if (err)
				callback("erreur");
